 ...
 * doesn't have to be explicitly included by the user.
 */
 ...
 *
 * The structure also contains `next`, and `prev` pointers to traverse
 * in the list in both directions.
 */
 ...
 * not allocate a particle manually. He/She should always use msm4g_particle
 * functions to handle the particles.
 */
 ...
 */
typedef struct Bin {
  I3Vector index;         /**< The index of the bin belong each axis */
  int      cantorindex;   /**< Cantor index corresponding to index variable */
  LinkedList *neighbors;  /**< The list of neighbor bins containing at
                                 least one particle */
...
typedef struct SimulationParameters {
  ...
  double    kmax;    /**< Choose k s.t. |invA *k| < kmax */
 ...
 *
 * This AbstractGrid structure defines the interfaces that the grid
 * implementations should follow. Its main aim is to hide the details of
 * different grid implementations as much as possible and increase the
 * code reuse.

 * Independent from the implementation, a 3D simulation domain is repeatedly
 * divided into grids whose nodes are stored is internal data structures of the
 * implementation grid object such as DenseGrid or SparseGrid.
 * The visualization of an AbstractGrid is shown below. In this 2D example
 * (3D case is the straightforward generalization) there are nx  and ny grid
 * nodes in each axis and the lattice spacing is h.
 *
 @verbatim
 (ny-1)*h +=======+=======+=======+=======+
          |       |       |       |       |
...
typedef struct AbstractGrid {
   ...
   * a one.
   */
  struct AbstractGrid * (*constructor) (int nx,int ny,int nz,
   ...
   */
  void   (*setElement)  (struct AbstractGrid *grid,int i,int j,int k,
      double value);
   ...
   */
  double (*innerProduct) (struct AbstractGrid *grid1,
      struct AbstractGrid *grid2);
...
typedef struct Simulation {
  struct SimulationParameters *parameters; /**< @brief All of the parameters
  of the algorithm. */
  struct SimulationBox        *box;        /**< Geometry of the simulation. */
  Particle                    *particles;  /**< The collection of the particles
  in the SimulationBox */
